...
import { DelegationType, PricingTierId } from '../context/RegistrationContext';
import { SecureFileUpload, UploadedDocumentMeta } from '../components/SecureFileUpload';
...
export function RegisterPage() {
  ...
  // Form State - Automatically applies date-based tier
  const autoTier = getAutoPricingTier();
  const [delegationType, setDelegationType] = useState<DelegationType>('school_delegation');
  const [tier, setTier] = useState<PricingTierId>(autoTier.id);
  ...
  // Synchronize members array when delegationSize changes
  useEffect(() => {
    const requiredMemberCount = delegationSize - 1;
    setMembers((prev) => {
      const current = [...prev];
  ...
  // Reset module selection based on category
  useEffect(() => {
    const isInd =
  ...
  const handleStep1Next = () => {
    ...
    if (isIndividual) {
      if (!indFullName.trim()) {
        setErrorMessage('Please provide the participant legal full name.');
        return;
      }
      if (!indPhone.trim()) {
        setErrorMessage('Please provide a valid contact/WhatsApp phone number.');
        return;
      }
      if (!indEmail.trim() || !indEmail.includes('@')) {
        setErrorMessage('Please provide a valid participant email address.');
        return;
      }
      if (!indCnic.trim()) {
        setErrorMessage('Please enter CNIC or B-Form number.');
        return;
      }
      if (!indCity.trim()) {
        setErrorMessage('Please enter participant city.');
        return;
      }
      ...
      if (!headName.trim()) {
        setErrorMessage('Please enter Head Delegate full legal name.');
        return;
      }
      if (!headPhone.trim() || !headEmail.trim()) {
        setErrorMessage('Please provide Head Delegate phone and email address.');
        return;
      }
      if (!headFileUploaded) {
      ...
      if (!advisorName.trim()) {
        setErrorMessage('Please provide Staff Advisor / Faculty Mentor name.');
        return;
      }
      if (!advisorPhone.trim() || !advisorEmail.trim()) {
        setErrorMessage('Please provide Staff Advisor phone and email.');
        return;
      }
      if (!city.trim()) {
      ...
      if (hasChaperone && !chaperoneName.trim()) {
        setErrorMessage('Please enter Chaperone Name or toggle Chaperone to No.');
        return;
      }
    }
    setStep(2);
  ...
  const handleStep2Next = () => {
    ...
    if (isIndividual) {
      if (selectedModules.length === 0) {
        setErrorMessage('Please choose at least 1 module (PnA or E-sports).');
        return;
      }
    } else {
      const hasCompulsory = COMPULSORY_MODULE_IDS.every((id) => selectedModules.includes(id));
      if (!hasCompulsory) {
        setErrorMessage('Core compulsory modules are mandatory for delegations.');
        return;
      }
  ...
  const toggleOptionalModule = (modId: string) => {
    ...
    if (COMPULSORY_MODULE_IDS.includes(modId as any)) return;
  ...
  return (
    <div className="min-h-screen pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto bg-black text-white">
      {/* Header */}
      <div className="text-center max-w-3xl mx-auto mb-10">
        <motion.div
          ...
          className="inline-flex items-center gap-2 px-5 py-1 rounded-full bg-purple-950/40 border border-purple-600/30 text-purple-400 text-xs font-semibold uppercase tracking-widest mb-3"
        >
          <Sparkles className="w-3.5 h-3.5" />
          <span>Sadiq Nexus '26 Official Secretariat Portal</span>
        </motion.div>
        ...
        <motion.p
          ...
          className="mt-3 text-sm sm:text-base text-zinc-300 leading-relaxed max-w-2xl mx-auto"
        >
          Built strictly to the prestigious ACSEC XI collegiate standard. Complete your institutional particulars, designate team delegates, and verify academic competition modules.
        </motion.p>
      </div>
      ...
      <div className="rounded-3xl bg-zinc-950 border border-zinc-800 shadow-2xl overflow-hidden">
        ...
        {step < 4 && (
          <div className="px-6 sm:px-8 py-4 bg-zinc-900/50 border-b border-zinc-800 flex items-center justify-between text-xs">
            <div
            ...
            >
              ...
              <span className="hidden sm:inline">Delegate Particulars</span>
            </div>
            <div className="w-8 sm:w-16 h-0.5 bg-zinc-800" />
            <div
              className={`flex items-center gap-2 cursor-pointer ${
            ...
            >
              ...
              <span className="hidden sm:inline">Summary & Audit</span>
            </div>
          </div>
        )}
        ...
        {/* Error Notification */}
        {errorMessage && (
          <div className="m-6 p-4 rounded-2xl bg-red-950/60 border border-red-500/40 text-red-200 text-xs flex items-center gap-3">
            ...
            <div className="space-y-6">
              <div>
                <span className="text-xs font-semibold uppercase tracking-wider text-purple-400">
                  Step 0 of 3 &bull; Delegation Pathway Selection
                </span>
                <h2 className="text-2xl font-serif font-bold text-white mt-1">
                  Choose Your Registration Pathway
                </h2>
                <p className="text-xs sm:text-sm text-zinc-300 mt-1 leading-relaxed">
                  Every delegation or participant must select an explicit classification before proceeding to roster details.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-zinc-300 mt-2 leading-relaxed">
                      Official institution-sponsored contingent representing a high school, grammar school, or college with an accredited faculty mentor.
                    </p>
                  </div>
                  <div className="mt-4 pt-3 border-t border-zinc-800/80 flex items-center justify-between text-[11px] text-purple-400 font-medium">
                    <span>Includes 4 Socials + Core Compulsory Tracks</span>
                    <CheckCircle2
                      className={`w-4 h-4 ${
                        delegationType === 'school_delegation' ? 'opacity-100' : 'opacity-0'
                      }`}
                    />
                  </div>
                </div>
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-zinc-300 mt-2 leading-relaxed">
                      Independent team assembled across multiple schools or independent collegiate academic groups without institutional sponsorship.
                    </p>
                  </div>
                  <div className="mt-4 pt-3 border-t border-zinc-800/80 flex items-center justify-between text-[11px] text-purple-400 font-medium">
                    <span>Includes 4 Socials + Core Compulsory Tracks</span>
                    <CheckCircle2
                      className={`w-4 h-4 ${
                        delegationType === 'private_delegation' ? 'opacity-100' : 'opacity-0'
                      }`}
                    />
                  </div>
                </div>
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-zinc-300 mt-2 leading-relaxed">
                      Single delegate enrolled at a school competing independently in PnA (Pure & Applied Sciences) and/or E-sports.
                    </p>
                  </div>
                  <div className="mt-4 pt-3 border-t border-zinc-800/80 flex items-center justify-between text-[11px] text-purple-400 font-medium">
                    <span>Exclusive Access: PnA & E-sports</span>
                    <CheckCircle2
                      className={`w-4 h-4 ${
                        delegationType === 'individual_school' ? 'opacity-100' : 'opacity-0'
                      }`}
                    />
                  </div>
                </div>
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-zinc-300 mt-2 leading-relaxed">
                      Independent competitor registering individually without school backing. Eligible exclusively for PnA and E-sports.
                    </p>
                  </div>
                  <div className="mt-4 pt-3 border-t border-zinc-800/80 flex items-center justify-between text-[11px] text-purple-400 font-medium">
                    <span>Exclusive Access: PnA & E-sports</span>
                    <CheckCircle2
                      className={`w-4 h-4 ${
                        delegationType === 'individual_private' ? 'opacity-100' : 'opacity-0'
                      }`}
                    />
                  </div>
                </div>
          ...
          {step === 1 && (
            <div className="space-y-6">
              <div>
                ...
                <h2 className="text-2xl font-serif font-bold text-white mt-1">
                  {isIndividual ? 'Individual Delegate Details' : 'Delegation Roster & Institutional Particulars'}
                </h2>
                <p className="text-xs sm:text-sm text-zinc-300 mt-1 leading-relaxed">
              ...
              {isIndividual ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-5 rounded-2xl bg-zinc-900/60 border border-zinc-800 space-y-4">
                    ...
                    {/* Delegation Size Selector: 5, 6, 7, or 8 delegates */}
                    <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          placeholder="Head Delegate Full Legal Name"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          placeholder="headdelegate@institution.edu"
                          className="w-full px-4 py-2.5 rounded-xl bg-black border border-zinc-800 text-xs text-white focus:outline-none focus:border-purple-500"
                        />
                      </div>
                      <div>
        ...
        {step < 4 && (
          <div className="px-6 py-4 sm:px-10 sm:py-5 border-t border-zinc-800 bg-zinc-900/60 flex items-center justify-between">
            {step > 0 ? (
              <button
                type="button"
                onClick={() => {
